...
//after-throwing은 비즈니스 메소드에서 발생된 예외 객체를 받아서 사후처리 로직을 구현함
@Service
@Aspect
public class AfterThrowingAdvice {
	...
	//컨테이너는 exceptionObj 변수에 비즈니스 메소드에서 발생된 에외 객체를 전달함
	@AfterThrowing(pointcut = "BoardPointcut.allPointcut()", throwing = "exceptionObj")
	public void exceptionLog(JoinPoint jp, Exception exceptionObj) {
		String methodName = jp.getSignature().getName();

		System.out.println("[예외 처리] "+methodName+"() 비즈니스 로직 수행 중 예외 발생");
